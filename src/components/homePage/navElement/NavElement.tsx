"use client";
import { NavElementType } from "@/src/lib/types/homePageTypes";
import Image from "next/image";
import { useRouter } from "next/navigation";

type NavElementProps = {
  nav: NavElementType;
};

export default function NavElement({ nav }: NavElementProps) {
  
  const navigate = useRouter();

  return (
    <div
      onClick={() => navigate.replace("/manage/" + nav.goTo)}
      key={nav.id}
      className="bg-surface-2/50 backdrop-blur-lg border-2 border-border w-40 h-40 p-4 flex justify-center items-center rounded-2xl hover:scale-110 transition-all ease-in-out"
    >
      <div className="flex justify-center items-center flex-col gap-4">
        <div className="invert w-14 h-14">
          <Image alt={"icon" + nav.id} src={nav.icon} fill />
        </div>
        <p className="text-xl">{nav.title}</p>
      </div>
    </div>
  );
}
